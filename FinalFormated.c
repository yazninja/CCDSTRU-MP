...
void gameInit(char P[][4], Player Ord[], Player Cha[], Player Free[])
{
    ...
    for (row = 0; row < 4; row++)
        Ord[row].col = 0; Ord[row].row = 0; 
    for (row = 0; row < 11; row++)
        Cha[row].col = 0; Cha[row].row = 0;  
} 
...
{ 
    int row, col, ctr = 0, k = 0, PCopy [4][4]; 
    Player FreeCopy [16]; 
    //initialize pCopy 
    for (row=0; row < 4; row ++) 
        for (col=0; col < 4; col++) PCopy[row][col] = 0; 
    //initialize FreeCopy 
    for (row = 0; row < 4; row ++)
        for (col = 0; col < 4; col ++) 
            if (PCopy[row][col]==0) 
...
{ 
    ...
    for(i=0;i<4;i++)
        if(nowPlayer[i].row==row&&nowPlayer[i].col==col) 
            nowPlayer[i].row=0; nowPlayer[i].col=0; 
} 
...
{ 
    ...
    { 
        ...
        removeFree(Free, row, col); P[row-1][col-1]='1'; 
        *turn = 0; 
        (*nElem)++; 
    } 
...
{ 
    ...
    { 
        ...
        for (i=0; i < 16; i++) 
            // check if there is a value in FreeCopy 
...
{ 
    ...
    int row, col, turn =1, over = 0, ordCtr=0, chaCtr = 0; 
